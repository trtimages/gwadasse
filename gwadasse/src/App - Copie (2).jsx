...
function computeDecision(reports, type) {
  const now = Date.now();
  const WINDOW = 2 * 60 * 60 * 1000;

  const recent = reports.filter(
    (r) => r.type === type && now - r.ts <= WINDOW
  );

  if (recent.length === 0) {
    return {
      level: 0,
      label: "Pas assez d’infos récentes",
      reliability: "faible",
      count: 0,
    };
  }
...
const recentReports = reports.filter((r) => r.beachId === beach.id);

const swimDecision = computeDecision(recentReports, "swim");
const rainDecision = computeDecision(recentReports, "rain");
const sargDecision = computeDecision(recentReports, "sargasses");
    ...
    return (
      <div style={decisionBox}>
        ...
        <div style={{ color: "#666", marginTop: 4 }}>
          Pas assez de signalements récents
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={decisionBox}>
      ...
      <div style={{ color: "#666", marginTop: 4, fontSize: 13 }}>
        Fiabilité {decision.reliability} — {decision.count} signalement{decision.count > 1 ? "s" : ""} (2 h)
      </div>
    </div>
  );
}
